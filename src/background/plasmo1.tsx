...
function DisplayTipComponent(appConfig) {
...
// 移除小框函数
    ...
    function parseShortCatStr(shortCatStr: string): ShortCat {
        const shortCat = new ShortCat();
        const shortCatStrArr = shortCatStr.split('+');
        for (let i = 0; i < shortCatStrArr.length; i++) {
            const key = shortCatStrArr[i];
            if (key == 'ctrl') {
                shortCat.ctrlKey = true;
            } else if (key == 'shift') {
                shortCat.shiftKey = true;
            } else if (key == 'alt') {
                shortCat.altKey = true;
            } else if (key == 'meta') {
                shortCat.metaKey = true;
            } else {
                shortCat.key = key;
            }
        }
    ...
    const displayTipShortCat = keymapConfig.displayTipShortCat;
    const shortCat = parseShortCatStr(displayTipShortCat);
    document.addEventListener('keydown', function (event) {
        if (event.ctrlKey === shortCat.ctrlKey
            && event.shiftKey === shortCat.shiftKey
            && event.altKey === shortCat.altKey
            && event.metaKey === shortCat.metaKey
            && event.key === shortCat.key) {
            displayTip();
